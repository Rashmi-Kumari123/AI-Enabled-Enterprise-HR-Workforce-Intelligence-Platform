...
export function ManagerPerformancePage() {
  ...
  const { employees, draftReview, reviews, feedback, selectedEmployee, isLoading, error, createReview, isCreating,createError, updateReview, setRatings,
     submitReview, isSubmitting, inviteFeedback, refetch } = useManagerPerformance(selectedEmployeeId);
  useEffect(() => {
  ...
  useEffect(() => {
    if (draftReview?.goals) {
      setGoals(draftReview.goals)
    }
    if (draftReview?.summaryComment) {
      setSummaryComment(draftReview.summaryComment)
    }
  }, [draftReview?.id])
  ...
  async function handleCreateReview() {
    ...
    try {
      ...
      // createError from hook is shown in the banner
    }
  }
            ...
            <CardContent className="max-h-[420px] space-y-1 overflow-y-auto">
              {employees.map((employee) => (
                ...
                >
                  ...
                  <span className={cn('text-xs', employee.id === selectedEmployeeId ? 'text-white/80' : 'text-muted-foreground')}>
                    {employee.email}
                  </span>
                </button>
              ))}
